...
- **后端模块 `backend/metadata_registry.py`**：14 条路径 / 17 个接口操作（overview / tree / store / reseed / products / subsystems / presets / presets/{id} / presets/{id}/bom / bom/items / device-catalog），含节点编号与 BOM 条目校验、删除保护、原子写与 RLock 串行化。
